...
void main() {
  vec2 uv = vUv;
  // Keep the CRT artwork at a stable 16:9 scale. On narrow screens this
  // samples a narrower slice of the field, so the sides crop instead of the
  // signal stretching vertically.
  float viewportAspect = uResolution.x / max(uResolution.y, 1.0);
  float referenceAspect = 16.0 / 9.0;
  if (viewportAspect < referenceAspect) {
    uv.x = (uv.x - 0.5) * (viewportAspect / referenceAspect) + 0.5;
  } else {
    uv.y = (uv.y - 0.5) * (referenceAspect / viewportAspect) + 0.5;
  }
...
export default function CRTWarp({
  ...
  useEffect(() => {
    ...
    const render = now => {
      ...
      if (!pausedRef.current) pointerCurrentRef.current.lerp(pointerTargetRef.current, 0.08);
  ...
  return <div ref={containerRef} className={`crt-warp-container ${className || ''}`} style={{ backgroundColor, ...style }} aria-hidden="true" />;
}
